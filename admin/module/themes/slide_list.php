...
$pagesize = 20;
$id = $_GET["id"]; 
$count_cates = $db->single("SELECT count(*) FROM slide");
...
$cates = $db->fetch("SELECT * FROM slide ORDER BY ".$order.$db->limit($pagesize, $page));
...
$categories = $db->fetch("SELECT * FROM slide order by id Asc");
?>
...
<div id="warpper">
<div id="submenu"><a href="slide_process.php">+เพิ่มสไลด์</a></div>
...
<tr><td align='left'><?=$categoriess["title"]?></td><td><div align='center'>
			   <a href="slide_process.php?event=edit&id=<?=$categoriess['id']?>"><img src="../../images/edit-icon.png" width="16" height="16" border="0" title="แก้ไขข้อมูล" /></a><a href="slide_list.php?event=delete&id=<?=$categoriess['id']?>" onclick="return confirm('Are you sure?');"><img src="../../images/bin.gif" width="16" height="16" border="0"  title="ลบข้อมูล"/></a></div> </td></tr>
               	<? }?>
</table>
